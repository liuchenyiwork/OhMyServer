...
void *accept_request(void *arg)//接收连接，收到连接请求
{
...
int startup(u_short *port)//开启服务
{
    ...
    if ((setsockopt(httpd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on))) < 0)  //设置选项
    {  
...
{
    int server_sock = -1;
    u_short port = 4000;        //设置监听的端口
    int client_sock = -1;       //客户端fd变量
    struct sockaddr_in client_name;//保存客户端的地址(in表示ipv4)
    socklen_t  client_name_len = sizeof(client_name);//
    pthread_t newthread;

    server_sock = startup(&port);//1.启动服务，返回的是监听socket的fd
